...
class IOSVoiceToTextParser: VoiceToTextParser, ObservableObject {
    ...
    private var micObserver = MicropthonePowerObserver()
    ...
    func startListening(languageCode: String) {
        ...
        let supportedLocale = SFSpeechRecognizer
            .supportedLocales()
            .contains(chosenLocale) ? chosenLocale : Locale.init(identifier: "en-US")
            ...
            SFSpeechRecognizer.requestAuthorization { [weak self] status in
                DispatchQueue.main.async {
                    if status != .authorized {
                        self?.updateState(error: "You need to grand permission to transcribe audio.")
                        self?.stopListening()
                        return
                    }
                    onGranted()
